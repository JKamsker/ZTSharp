...
- [x] 5.3 OS UDP receive loop survives socket exceptions: new tests in `ZTSharp.Tests`
- [x] 5.4 Windows IOCTL behavior verified: Windows-only tests or diagnostics
- [x] 5.5 Dual-mode bind fallback resilience (IPv6-only before IPv4): new tests in `ZTSharp.Tests`
- [x] 5.3 Discovery replies don't block receive loop: new tests in `ZTSharp.Tests`
- [x] 5.6 OS-UDP peer registry bounded (TTL/eviction): new tests in `ZTSharp.Tests`
