...
internal sealed class UserSpaceTcpServerConnection : IAsyncDisposable
{
    ...
    public async ValueTask WriteAsync(ReadOnlyMemory<byte> buffer, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            while (offset < buffer.Length)
            {
                ...
                var maxSegmentLength = Math.Min((int)_mss, buffer.Length - offset);
                var segmentLength = Math.Min(maxSegmentLength, (int)remoteWindow);
                if (segmentLength == 0)
                {
                    continue;
                }
                ...
                await SendTcpWithRetriesAsync(
                        seq: seq,
                        ack: _recvNext,
                        flags: TcpCodec.Flags.Psh | TcpCodec.Flags.Ack,
    ...
    public async ValueTask DisposeAsync()
    {
        ...
        try
        {
            ...
            SignalRemoteSendWindowWaiters(new ObjectDisposedException(nameof(UserSpaceTcpServerConnection)));
        ...
        while (!token.IsCancellationRequested)
        {
            ...
            if (!_connected)
            {
                if ((flags & TcpCodec.Flags.Syn) != 0 && (flags & TcpCodec.Flags.Ack) == 0)
                {
                    ...
                    var synAckOptions = TcpCodec.EncodeMssOption(_mss);
                    await SendTcpAsync(
                            seq: _sendUna,
                            ack: _recvNext,
    ...
    {
        private readonly UserSpaceTcpServerConnection _client;

        public TcpStream(UserSpaceTcpServerConnection client)
        {
            _client = client;
        }
