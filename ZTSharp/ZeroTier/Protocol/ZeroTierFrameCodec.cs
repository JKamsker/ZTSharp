...
internal static class ZeroTierFrameCodec
{
        ...
        out ReadOnlySpan<byte> frame)
    {
        ...
        if ((flags & 0x01) != 0)
        {
            if (!ZeroTierCertificateOfMembershipCodec.TryGetSerializedLength(payload.Slice(offset), out var comLen))
            {
                return false;
            }
        ...
        to = ZeroTierMac.Read(payload.Slice(offset, 6));
        offset += 6;

        from = ZeroTierMac.Read(payload.Slice(offset, 6));
        offset += 6;
