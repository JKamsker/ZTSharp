...
internal static class ZtZeroTierIdentityHashcash
{
    ...
    {
        if (publicKey.Length != ZtZeroTierIdentity.PublicKeyLength)
        {
            throw new ArgumentException($"Public key must be {ZtZeroTierIdentity.PublicKeyLength} bytes.", nameof(publicKey));
        }
