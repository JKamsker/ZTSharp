...
[global::System.Diagnostics.CodeAnalysis.SuppressMessage(
    ...
    Justification = "ZtManagedSocket does not own the ZtZeroTierSocket instance.")]
public sealed class ZtManagedSocket : IAsyncDisposable, IDisposable
{
    private readonly ZtZeroTierSocket _zt;
    ...
    private ZtZeroTierTcpListener? _listener;
    private ZtZeroTierUdpSocket? _udp;

    public ZtManagedSocket(ZtZeroTierSocket zeroTier, AddressFamily addressFamily, SocketType socketType, ProtocolType protocolType)
    {
    ...
    private ZtManagedSocket(ZtZeroTierSocket zeroTier, IPEndPoint localEndPoint, IPEndPoint? remoteEndPoint, Stream connectedStream)
    {
        _zt = zeroTier;
    ...
    public ZtManagedSocket Accept()
        => AcceptAsync().AsTask().GetAwaiter().GetResult();

    public async ValueTask<ZtManagedSocket> AcceptAsync(CancellationToken cancellationToken = default)
    {
        cancellationToken.ThrowIfCancellationRequested();
        ObjectDisposedException.ThrowIf(_disposed, this);
        EnsureTcp();
        ...
        return new ZtManagedSocket(_zt, local, remoteEndPoint: null, connectedStream: stream);
    }
