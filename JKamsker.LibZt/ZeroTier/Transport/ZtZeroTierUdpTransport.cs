...
internal sealed class ZtZeroTierUdpTransport : IAsyncDisposable
{
    private const int WindowsSioUdpConnReset = unchecked((int)0x9800000C);

    private readonly UdpClient _udp;
    private readonly Channel<ZtZeroTierUdpDatagram> _incoming;
    private readonly Action<string>? _log;
    private readonly CancellationTokenSource _cts = new();
    private readonly Task _receiverLoop;
    private bool _disposed;

    public ZtZeroTierUdpTransport(int localPort = 0, bool enableIpv6 = true, Action<string>? log = null)
    {
        _log = log;
        ...
        _incoming = Channel.CreateUnbounded<ZtZeroTierUdpDatagram>();
    ...
    public ValueTask<ZtZeroTierUdpDatagram> ReceiveAsync(CancellationToken cancellationToken = default)
    {
        ObjectDisposedException.ThrowIf(_disposed, this);
        return _incoming.Reader.ReadAsync(cancellationToken);
    }

    public async ValueTask<ZtZeroTierUdpDatagram> ReceiveAsync(
        TimeSpan timeout,
    ...
    private async Task ProcessReceiveLoopAsync()
    {
        ...
        while (!token.IsCancellationRequested)
        {
            ...
            if (!_incoming.Writer.TryWrite(new ZtZeroTierUdpDatagram(
