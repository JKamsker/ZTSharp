...
internal static class ZtZeroTierPacketCrypto
{
    ...
    public static void Armor(Span<byte> packet, ReadOnlySpan<byte> key, bool encryptPayload)
    {
        ...
        ZtZeroTierSalsa20.GenerateKeyStream12(mangledKey, packet.Slice(0, 8), keystream);
    ...
    private static void SetCipher(Span<byte> packet, byte cipher)
    {
        ...
        if (cipher == CipherC25519Poly1305Salsa2012)
        {
            flags |= ZtZeroTierPacketHeader.FlagEncryptedDeprecated;
        }
        else
        {
            flags &= unchecked((byte)~ZtZeroTierPacketHeader.FlagEncryptedDeprecated);
        }
