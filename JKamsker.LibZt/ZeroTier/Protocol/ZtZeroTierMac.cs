...
/// </summary>
internal readonly record struct ZtZeroTierMac(ulong Value)
{
    public const ulong MaxValue = 0xFFFFFFFFFFFFUL;

    public static readonly ZtZeroTierMac Broadcast = new(MaxValue);
    ...
    public static ZtZeroTierMac FromAddress(ZtNodeId nodeId, ulong networkId)
    {
        if (nodeId.Value > ZtNodeId.MaxValue)
        {
        ...
        return new ZtZeroTierMac(m & MaxValue);
    }

    public ZtNodeId ToAddress(ulong networkId)
    {
        ...
        return new ZtNodeId(a & ZtNodeId.MaxValue);
    }
    ...
    public static ZtZeroTierMac Read(ReadOnlySpan<byte> source)
    {
        if (source.Length < 6)
        {
        ...
        var value =
            ...
            ((ulong)source[4] << 8) |
            source[5];
        return new ZtZeroTierMac(value);
    }
