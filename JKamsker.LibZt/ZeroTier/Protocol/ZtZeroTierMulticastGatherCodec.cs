...
internal static class ZtZeroTierMulticastGatherCodec
{
    public static byte[] EncodeRequestPayload(
        ulong networkId,
        in ZtZeroTierMulticastGroup group,
        uint gatherLimit,
    ...
    public static bool TryParseOkPayload(
        ...
        out ulong networkId,
        out ZtZeroTierMulticastGroup group,
        out uint totalKnown,
        out ZtNodeId[] members)
    {
        networkId = 0;
        group = default;
        totalKnown = 0;
        members = Array.Empty<ZtNodeId>();
        ...
        var mac = ZtZeroTierMac.Read(payload.Slice(8, 6));
        var adi = BinaryPrimitives.ReadUInt32BigEndian(payload.Slice(14, 4));
        group = new ZtZeroTierMulticastGroup(mac, adi);
