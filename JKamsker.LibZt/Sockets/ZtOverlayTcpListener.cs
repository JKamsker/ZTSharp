...
/// </summary>
public sealed class ZtOverlayTcpListener : IAsyncDisposable
{
    ...
    private readonly Channel<ZtOverlayTcpClient> _acceptQueue;
    ...
    public ZtOverlayTcpListener(ZtNode node, ulong networkId, int localPort)
    {
        ...
        _acceptQueue = Channel.CreateUnbounded<ZtOverlayTcpClient>();

        _node.RawFrameReceived += OnFrameReceived;
    }

    public ValueTask<ZtOverlayTcpClient> AcceptTcpClientAsync(CancellationToken cancellationToken = default)
    ...
    private void OnFrameReceived(in ZtRawFrame frame)
    {
        ...
        ZtOverlayTcpClient? accepted = null;
        try
        {
            accepted = new ZtOverlayTcpClient(_node, _networkId, _localPort, remoteNodeId, remotePort, connectionId);
