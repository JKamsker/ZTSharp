...
public sealed class ZtOverlayHttpMessageHandlerOptions
{
    /// <summary>
    /// Optional mapping of IP addresses to node ids.
    /// </summary>
    public ZtOverlayAddressBook? AddressBook { get; init; }
...
/// <summary>
/// HttpClient handler that dials overlay TCP streams (not OS TCP) using <see cref="ZtOverlayTcpClient"/>.
/// </summary>
public sealed class ZtOverlayHttpMessageHandler : DelegatingHandler
{
    private readonly ZtNode _node;
    private readonly ulong _networkId;
    private readonly ZtOverlayHttpMessageHandlerOptions _options;
    private int _nextLocalPort;

    public ZtOverlayHttpMessageHandler(ZtNode node, ulong networkId, ZtOverlayHttpMessageHandlerOptions? options = null)
    {
        ArgumentNullException.ThrowIfNull(node);
        _node = node;
        _networkId = networkId;
        _options = options ?? new ZtOverlayHttpMessageHandlerOptions();
    ...
    private async ValueTask<Stream> ConnectOverlayAsync(SocketsHttpConnectionContext context, CancellationToken cancellationToken)
    {
        ...
        var client = new ZtOverlayTcpClient(_node, _networkId, localPort);
        try
        {
    ...
    private ulong ResolveNodeId(string host)
    {
        ...
        if (custom.HasValue)
        {
            if (custom.Value == 0 || custom.Value > ZtNodeId.MaxValue)
            {
    ...
    {
        private readonly ZtOverlayTcpClient _client;
        private readonly Stream _inner;
        private int _disposed;

        public OwnedZtOverlayStream(ZtOverlayTcpClient client)
        {
            _client = client;
