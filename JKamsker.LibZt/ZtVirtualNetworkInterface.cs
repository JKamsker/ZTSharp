...
public sealed class ZtVirtualNetworkInterface : IAsyncDisposable
{
    ...
    public ZtVirtualNetworkInterface(ZtNode node, ulong networkId)
    {
        ArgumentNullException.ThrowIfNull(node);
        _node = node;
        _networkId = networkId;
        _localNodeId = node.NodeId.Value;
        _incoming = Channel.CreateUnbounded<ZtIpPacket>();
    ...
    public ValueTask<ZtIpPacket> ReceivePacketAsync(CancellationToken cancellationToken = default)
    ...
    private void OnFrameReceived(in ZtRawFrame frame)
    {
        ...
        _incoming.Writer.TryWrite(new ZtIpPacket(
            frame.SourceNodeId,
