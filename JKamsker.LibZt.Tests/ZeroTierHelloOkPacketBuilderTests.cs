...
public sealed class ZeroTierHelloOkPacketBuilderTests
{
    ...
    public void BuildPacket_EncodesOkHello()
    {
        ...
        ZeroTierC25519.Agree(identity.PrivateKey!, identity.PublicKey, sharedKey);

        var destination = new NodeId(0x0102030405);
        ...
        var packetBytes = ZeroTierHelloOkPacketBuilder.BuildPacket(
            packetId: 2,
            destination,
            source,
            inRePacketId,
        ...
        Assert.True(ZeroTierPacketCrypto.Dearmor(packetBytes, sharedKey));
        Assert.Equal(ZeroTierVerb.Ok, (ZeroTierVerb)(packetBytes[27] & 0x1F));

        var payload = packetBytes.AsSpan(ZeroTierPacketHeader.Length);

        Assert.Equal(ZeroTierVerb.Hello, (ZeroTierVerb)(payload[0] & 0x1F));
        ...
        Assert.Equal(ZeroTierHelloClient.AdvertisedProtocolVersion, payload[17]);
        Assert.Equal(ZeroTierHelloClient.AdvertisedMajorVersion, payload[18]);
        Assert.Equal(ZeroTierHelloClient.AdvertisedMinorVersion, payload[19]);
        Assert.Equal(ZeroTierHelloClient.AdvertisedRevision, BinaryPrimitives.ReadUInt16BigEndian(payload.Slice(20, 2)));

        Assert.True(ZeroTierInetAddressCodec.TryDeserialize(payload.Slice(22), out var parsedSurface, out var read));
