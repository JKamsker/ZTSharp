...
public sealed class ZeroTierInetAddressCodecTests
{
    [Fact]
    public void Serialize_Deserialize_RoundTripsIpv4()
    {
        var endpoint = new IPEndPoint(IPAddress.Parse("10.0.0.1"), 9993);
        var buffer = new byte[ZeroTierInetAddressCodec.GetSerializedLength(endpoint)];

        var written = ZeroTierInetAddressCodec.Serialize(endpoint, buffer);
        Assert.Equal(buffer.Length, written);

        Assert.True(ZeroTierInetAddressCodec.TryDeserialize(buffer, out var parsed, out var bytesRead));
    ...
    {
        Assert.True(ZeroTierInetAddressCodec.TryDeserialize([0], out var endpoint, out var bytesRead));
    ...
    public void Deserialize_CanSkipUnknownTypes()
    {
        ...
        Assert.True(ZeroTierInetAddressCodec.TryDeserialize(data, out var endpoint, out var bytesRead));
