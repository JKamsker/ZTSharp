...
public sealed class ZtZeroTierPacketCryptoTests
{
    ...
    public void Salsa20_12_Matches_Upstream_TestVector()
    {
        ...
        ZtZeroTierSalsa20.GenerateKeyStream12(S2012Tv0Key, S2012Tv0Iv, keystream);
    ...
    public void C25519_Agree_Matches_Upstream_TestVector()
    {
        ...
        ZtZeroTierC25519.Agree(C25519Tv0Priv1, C25519Tv0Pub2, agreement);
    ...
    public void PacketCrypto_Roundtrips_With_And_Without_Encryption()
    {
        var header = new ZtZeroTierPacketHeader(
            ...
            Flags: 0,
            Mac: 0,
            VerbRaw: (byte)ZtZeroTierVerb.Echo);

        var payload = "test-payload"u8.ToArray();
        var plain = ZtZeroTierPacketCodec.Encode(header, payload);
        ...
        ZtZeroTierPacketCrypto.Armor(encrypted, key, encryptPayload: true);
        Assert.True(ZtZeroTierPacketCrypto.Dearmor(encrypted, key));
        Assert.True(encrypted.AsSpan(27).SequenceEqual(plain.AsSpan(27)));

        // MAC-only + verify
        var macOnly = (byte[])plain.Clone();
        ZtZeroTierPacketCrypto.Armor(macOnly, key, encryptPayload: false);
        Assert.True(ZtZeroTierPacketCrypto.Dearmor(macOnly, key));
        ...
        ZtZeroTierPacketCrypto.Armor(shouldFail, key, encryptPayload: true);
        Assert.False(ZtZeroTierPacketCrypto.Dearmor(shouldFail, wrongKey));
    }
