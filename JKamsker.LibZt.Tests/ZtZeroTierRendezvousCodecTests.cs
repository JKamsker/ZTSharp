...
public sealed class ZtZeroTierRendezvousCodecTests
{
    [Fact]
    public void TryParse_ParsesIpv4()
    {
        ...
        Assert.True(ZtZeroTierRendezvousCodec.TryParse(payload, out var rendezvous));
        Assert.Equal((byte)0x00, rendezvous.Flags);
        Assert.Equal(new ZtNodeId(0x1122334455UL), rendezvous.With);
    ...
    public void TryParse_ParsesIpv6()
    {
        ...
        Assert.True(ZtZeroTierRendezvousCodec.TryParse(payload, out var rendezvous));
        Assert.Equal(new ZtNodeId(0xAABBCCDDEEUL), rendezvous.With);
    ...
    {
        Assert.False(ZtZeroTierRendezvousCodec.TryParse(new byte[1 + 5 + 2], out _));
    }
}
