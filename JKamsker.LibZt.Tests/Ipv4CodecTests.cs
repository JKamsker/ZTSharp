...
public sealed class Ipv4CodecTests
{
    ...
    public void Encode_Parse_RoundTrips()
    {
        ...
        var packet = Ipv4Codec.Encode(src, dst, protocol: 0x11, payload, identification: 0x1234);

        Assert.True(Ipv4Codec.TryParse(packet, out var parsedSrc, out var parsedDst, out var proto, out var parsedPayload));
