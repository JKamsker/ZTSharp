...
public sealed class ZtIcmpv6CodecTests
{
    [Fact]
    public void ComputeChecksum_WhenApplied_ValidatesToZero()
    {
        ...
        var checksum = ZtIcmpv6Codec.ComputeChecksum(src, dst, message);
        BinaryPrimitives.WriteUInt16BigEndian(message.AsSpan(2, 2), checksum);

        Assert.Equal((ushort)0, ZtIcmpv6Codec.ComputeChecksum(src, dst, message));
    }
}
