...
public sealed class ZtVirtualNetworkInterfaceTests
{
    [Fact]
    public async Task InMemoryVirtualInterface_DeliversPacket()
    {
        ...
        await using var iface1 = new ZtVirtualNetworkInterface(node1, networkId);
        await using var iface2 = new ZtVirtualNetworkInterface(node2, networkId);
