...
public sealed class ZtZeroTierLz4Tests
{
    [Fact]
    public void Decompress_LiteralsOnly_Works()
    {
        ...
        Assert.True(ZtZeroTierLz4.TryDecompress(compressed, output, out var written));
    ...
    public void PacketCompression_UncompressesPayload_AndClearsFlag()
    {
        ...
        packet[27] = (byte)(ZtZeroTierPacketHeader.VerbFlagCompressed | (byte)ZtZeroTierVerb.Nop);
        packet[28] = 0x30;
        packet[29] = (byte)'a';
        packet[30] = (byte)'b';
        packet[31] = (byte)'c';

        Assert.True(ZtZeroTierPacketCompression.TryUncompress(packet, out var uncompressed));
        Assert.Equal(28 + 3, uncompressed.Length);
        Assert.Equal((byte)ZtZeroTierVerb.Nop, (byte)(uncompressed[27] & 0x1F));
        Assert.Equal(0, uncompressed[27] & ZtZeroTierPacketHeader.VerbFlagCompressed);
