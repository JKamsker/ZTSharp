...
public sealed class ZtOverlayTcpTests
{
    [Fact]
    public async Task InMemoryOverlayTcp_EchoesPayload()
    {
        ...
        await using var serverNode = new ZtNode(new ZtNodeOptions
        {
            StateRootPath = Path.Combine(Path.GetTempPath(), "zt-node-" + Guid.NewGuid()),
            StateStore = new MemoryZtStateStore()
        });

        await using var clientNode = new ZtNode(new ZtNodeOptions
        {
            StateRootPath = Path.Combine(Path.GetTempPath(), "zt-node-" + Guid.NewGuid()),
            StateStore = new MemoryZtStateStore()
        });
        ...
        await using var listener = new ZtOverlayTcpListener(serverNode, networkId, 20000);
        var acceptTask = listener.AcceptTcpClientAsync().AsTask();

        await using var client = new ZtOverlayTcpClient(clientNode, networkId, 20001);
