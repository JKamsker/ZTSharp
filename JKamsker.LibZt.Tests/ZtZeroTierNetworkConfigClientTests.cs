...
public sealed class ZtZeroTierNetworkConfigClientTests
{
    ...
    public async Task FetchAsync_CanObtainAssignedIps_FromSignedChunk()
    {
        ...
        await using var controllerUdp = new ZtZeroTierUdpTransport(localPort: 0, enableIpv6: true);
        ...
        var planet = new ZtZeroTierWorld(
            ...
            roots: new[]
            {
                new ZtZeroTierWorldRoot(rootIdentity, new[] { rootUdp.LocalEndpoint })
            });
    ...
    private static async Task RunRootAsync(
        ZtZeroTierUdpTransport rootUdp,
        ...
        CancellationToken cancellationToken)
    {
        ...
        ZtZeroTierC25519.Agree(rootIdentity.PrivateKey!, localIdentity.PublicKey, rootKey);
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            if (!ZtZeroTierPacketCodec.TryDecode(packet, out var decoded))
            {
                continue;
            }
            ...
            if (decoded.Header.Destination == rootIdentity.NodeId)
            {
                ...
                if (!ZtZeroTierPacketCrypto.Dearmor(authPacket, rootKey))
                {
                    continue;
                }
                ...
                else if (verb == ZtZeroTierVerb.Whois && localEndpoint is not null)
                {
                    var identityBytes = ZtZeroTierIdentityCodec.Serialize(controllerIdentity, includePrivate: false);
                    var okPayload = new byte[1 + 8 + identityBytes.Length];
                    okPayload[0] = (byte)ZtZeroTierVerb.Whois;
    ...
    private static async Task RunControllerAsync(
        ZtZeroTierUdpTransport controllerUdp,
        ZtZeroTierIdentity controllerIdentity,
        ZtZeroTierIdentity localIdentity,
        CancellationToken cancellationToken)
    {
        var key = new byte[48];
        ZtZeroTierC25519.Agree(controllerIdentity.PrivateKey!, localIdentity.PublicKey, key);
        ...
        while (!cancellationToken.IsCancellationRequested)
        {
            ...
            var authPacket = (byte[])packet.Clone();
            if (!ZtZeroTierPacketCrypto.Dearmor(authPacket, key))
            {
                continue;
            }
            ...
            var networkId = BinaryPrimitives.ReadUInt64BigEndian(authPacket.AsSpan(ZtZeroTierPacketHeader.Length, 8));
            ...
            okPayload[0] = (byte)ZtZeroTierVerb.NetworkConfigRequest;
            BinaryPrimitives.WriteUInt64BigEndian(okPayload.AsSpan(1, 8), decoded.Header.PacketId);
            ...
            var okPacket = ZtZeroTierPacketCodec.Encode(okHeader, okPayload);
            ZtZeroTierPacketCrypto.Armor(okPacket, key, encryptPayload: true);
    ...
    private static byte[] BuildHelloOkPayload(ulong helloPacketId, ulong helloTimestamp, IPEndPoint surface)
    {
        ...
        _ = ZtZeroTierInetAddressCodec.Serialize(surface, payload.AsSpan(22));
        return payload;
    }
    ...
    private static byte[] BuildDictionaryWithStaticIp(IPAddress address, int bits)
    {
        ...
        var inetLen = ZtZeroTierInetAddressCodec.GetSerializedLength(endpoint);
        var inet = new byte[inetLen];
        _ = ZtZeroTierInetAddressCodec.Serialize(endpoint, inet);
    ...
    private static byte[] BuildSignedConfigChunkPayload(ulong networkId, byte[] dictionaryBytes, byte[] controllerPrivateKey)
    {
        ...
        var signature = ZtZeroTierC25519.Sign(controllerPrivateKey, signatureMessage);
    ...
    private static ZtZeroTierIdentity CreateFastIdentity(ulong nodeId)
    {
        var privateKey = new byte[ZtZeroTierIdentity.PrivateKeyLength];
        RandomNumberGenerator.Fill(privateKey);

        var publicKey = new byte[ZtZeroTierIdentity.PublicKeyLength];
