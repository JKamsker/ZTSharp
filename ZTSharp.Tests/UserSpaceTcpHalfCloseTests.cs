...
public sealed class UserSpaceTcpHalfCloseTests
{
    [Fact]
    public async Task WriteAsync_AfterPeerFin_ThrowsIOException()
    {
        ...
        await Assert.ThrowsAsync<IOException>(() => client.WriteAsync(new byte[] { 1 }, cts.Token).AsTask());
    }
